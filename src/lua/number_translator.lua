--[[
代码来自：https://github.com/tswwe/my-rime-lua
...
local function speakMoney(str)
	local part = splitNumStr(str)
	local speakSym = speakLiterally(part.sym)
	local speakInt = speakIntOfficially(part.int, {[1]="仟"; [2]="佰"; [3]="拾"; [4]=""}, {[0]="零"; "壹"; "贰"; "叁" ;"肆"; "伍"; "陆"; "柒"; "捌"; "玖"}) .. "圆"
...
local function translator(input, seg, env)
	...
	if string.sub(input, 1, 1) == "=" then
		...
		if string.match(input2, "^[%+%-]?%d*%.?%d*$") then -- sadly, lua does not support regex like {0,4}
			...
			yield(Candidate("number", seg.start, seg._end, speakOfficially(input2), " 数读"))
			yield(Candidate("number", seg.start, seg._end, speakLiterally(input2), " 位读"))
